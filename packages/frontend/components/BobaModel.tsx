...
const BobaModel = (): JSX.Element => {
  const refContainer = useRef<HTMLDivElement>(null);
  const [renderer, setRenderer] = useState<any>(null);
  const [target] = useState(new THREE.Vector3(40, 4, 0));
  const [initialCameraPosition] = useState(
    new THREE.Vector3(6, 1, 20 * Math.cos(0.2 * Math.PI))
  );
  const [scene] = useState(new THREE.Scene());

  /* eslint-disable */
  useEffect(() => {
    const container = document.getElementById("boba");
